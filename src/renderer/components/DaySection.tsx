...
/** 周记中的只读日期分组，数组下标只用于区分允许存在的完全重复任务。 */
export function DaySection({ group }: { group: WeeklyDayGroup }) {
  return (
    <section
      className="min-w-0 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5"
    ...
    >
      <header className="mb-3 flex items-baseline justify-between gap-4 border-b border-stone-100 pb-2">
        <h3 className="font-semibold text-stone-900" id={`day-${group.date}`}>
          {group.weekdayLabel}
        </h3>
        <time className="text-xs tabular-nums text-stone-400" dateTime={group.date}>
      ...
      <ul className="space-y-2">
        {group.tasks.map((task, index) => (
          <li
            className="flex min-w-0 items-baseline gap-2 text-sm text-stone-700 sm:gap-3"
            key={`${group.date}:${index}:${task.content}`}
          >
