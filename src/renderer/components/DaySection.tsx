...
export function DaySection({ group }: { group: WeeklyDayGroup }) {
  return (
    <section className="min-w-0 rounded-2xl border border-stone-200 bg-white p-4 shadow-sm sm:p-5" aria-labelledby={`day-${group.date}`}>
      ...
      <ul className="space-y-2">
        {group.tasks.map((task, index) => (
          <li className="flex min-w-0 items-baseline gap-2 text-sm text-stone-700 sm:gap-3" key={`${group.date}:${index}:${task.content}`}>
