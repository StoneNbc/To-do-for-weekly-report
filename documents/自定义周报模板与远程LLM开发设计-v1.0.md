...
| 目标状态 | 核心设计已落地，待 P0 收尾与双平台验收      |

---

## 0. 实施状态快照

截至 2026-08-17，`ReportTemplateService`、`CredentialService`、`LlmEndpointPolicy`、`LlmHttpClient`、`OpenAICompatibleAgent`、报告草稿内存授权、设置页编辑与预览链路均已接入。连接测试已拆分为独立严格 IPC 输入，只传递 LLM 连接配置和可选临时密钥，并固定使用短消息、`temperature: 0` 与 `maxTokens: 8`。设置窗口的未保存状态由 Renderer 上报、Main 拦截原生关闭事件并由用户确认放弃。

当前主要技术债：`remote_consent_confirmed` 仍是全局布尔值，尚未绑定 origin 与隐私声明版本；三份文本与配置/凭据仍缺少完整跨文件事务或回滚；Windows safeStorage、安装包和真实服务兼容性仍待实机验证。
...
> 项目管理增量（2026-09-09，已实现，未发布）：[项目管理开发文档](项目管理开发文档-v1.0.md) §7 扩展本节白名单中的任务投影为独立 projectName 字段，并增加项目过滤、候选显式选择、预览与请求共用素材快照。项目名直接来自任务，不读取或上传 projects.txt；详情继续排除。候选默认关闭，Main 已校验服务 origin／隐私版本，旧布尔授权不能单独通过检查。
