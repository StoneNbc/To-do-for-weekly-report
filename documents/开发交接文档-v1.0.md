...
| 文档版本     | v1.2                                                       |
| 交接日期     | 2026-08-27                                                 |
| 当前应用版本 | `0.1.2` / 标签 `v0.1.2`                                    |
| 当前主分支   | `main`                                                     |
| 功能基线     | `089b1ef feat: add branded application icons`              |
...
```text
089b1ef feat: add branded application icons
```

行为：

- 使用透明便签勾选图作为应用、Dock、窗口和安装包图标。
- 提供 16 × 16 与 32 × 32 托盘资源，Electron 将 `@2x` 文件识别为高分辨率变体。
- 开发模式从仓库 `build/` 读取；打包后从 `process.resourcesPath` 读取。
- 图标加载失败时记录警告，托盘回退到内嵌占位图，不阻止应用启动。

实现位置：

```text
resources/app-icon-master.png
build/icon.png
build/tray-icon.png
