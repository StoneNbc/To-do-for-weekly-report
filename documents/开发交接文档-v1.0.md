...
| 当前主分支   | `main`                                                 |
| 功能基线     | `013ba44 improve settings close guard and connection test` |
...
```text
013ba44 improve settings close guard and connection test
```

行为：

- 修改模板、提示词、模型连接参数或 API Key 后，关闭设置窗口会提示继续编辑或放弃修改。
- 应用正式退出不重复拦截；用户确认放弃只放行一次关闭。
- 测试连接只提交 LLM 连接参数与可选临时密钥。
- 远程测试固定发送短消息，使用 `temperature: 0` 和 `maxTokens: 8`，不发送任务、模板或提示词。

实现位置：

```text
src/renderer/pages/SettingsPage.tsx
src/main/services/settingsCloseGuard.ts
src/main/services/reportSettingsService.ts
tests/renderer/settings-page.test.tsx
tests/unit/settingsCloseGuard.test.ts
tests/integration/services/reportSettingsService.test.ts
```

### 2.3 尚未完成或尚未充分验证

- 已提供正式品牌主图标和 1x/2x 托盘图标；electron-builder、应用窗口和托盘均已接入。
- macOS 构建未签名、未公证。
