...
| 文档版本     | v1.4                                                       |
| 交接日期     | 2026-09-04                                                 |
| 当前应用版本 | `0.1.4` / 发布标签 `v0.1.4`                                |
| 当前主分支   | `main`                                                     |
| 功能基线     | 待办详情、紧凑模式、贴边自动隐藏与全屏 Space 悬浮          |
| 技术栈       | Electron 37 + React 18 + TypeScript 5 + Tailwind CSS 3     |
| 支持目标     | macOS 11+、Windows 10+                                     |
| 产品状态     | 核心功能与可选远程 LLM 已实现；P0 收尾和正式分发准备未完成 |
...
1. 先读[根目录开发入口](../AGENTS.md)和[项目当前状态](./PROJECT_STATE.md)，已在上下文中的内容不重复读取。
2. 按根目录导航搜索本次涉及的代码、测试和文档章节；本文作为详细参考，无需全文预读。
3. 只有用户明确要求并行开发时，才查阅[多 Agent 策略](./多Agent并行开发策略-v1.0.md)相关章节。
4. 实质修改结束后覆盖更新状态摘要；架构或规则变化同步本文及所属设计的相关章节，不追加对话流水账。

本文版本、测试数字和发布信息为表头日期的历史记录；实时状态需核实当前代码与 Git，不以旧记录代替本次验收。
...
开始先检查 Git 状态，开发中按需运行最小相关检查；最终门禁默认由用户执行，分工以[协作开发流程](协作开发流程.md)及根目录 AGENTS.md 为准。本文后续“必须运行全量门禁”表示验收要求，不要求 Agent 默认代跑。无新修改或失败时不重复已通过的检查。纯文档修改只检查变更片段格式、链接与 `git diff --check`。
...
- 今日任务添加、完成、撤销、标题与多行详情编辑、删除。
- 详情默认收起、按任务独立展开，并随完成、归档、历史完成和重新打开保持。
- 便利贴可整体收成仅保留标题栏的紧凑模式，恢复时保留原尺寸和移动后位置。
- 紧凑状态保留日期导航、三个点菜单和展开按钮；打开菜单时先恢复完整窗口。
- 完成时间记录为本地 `HH:mm`。
- 已完成区域折叠和展开。
...
- 左侧、右侧和顶部贴边自动隐藏，隐藏后保留 2 DIP 提示条并支持独立颜色设置。
- macOS 使用原生 panel 跨普通桌面和全屏 Space 显示便利贴。
- TemplateAgent 周报文本生成。
- 自定义本地记录模板、远程完整周报模板、提示词与本地预览。
- DeepSeek、通义千问、Kimi、智谱、本地和自定义 OpenAI 兼容服务配置。
- API Key 系统加密存储、按 origin 读取及明文 HTTP 风险确认。
- 远程生成草稿、保存前预览编辑和系统对话框导出。
- 设置页未保存修改关闭提醒和最小化连接测试请求。
- 系统保存对话框导出 TXT。
- 导出后打开文件或在 Finder 中定位。
- Renderer 生产网络拦截与 Main Process 受控远程 LLM 出站。
- macOS arm64 未签名 `.app` 目录包构建和真实 GUI 冒烟。

### 2.2 最近一次功能修改

`v0.1.4` 在 `v0.1.3` 的待办详情与紧凑模式基础上增加桌面悬浮能力：

- 拖到屏幕左侧、右侧或顶部后自动吸附，鼠标离开 500 ms 后收起。
- 隐藏后原生窗口缩为 2 DIP 提示条，避免 macOS 切换 Space 后出现大块白色空窗。
- 提示条颜色支持 6 个预设和系统自定义取色器，独立保存为 `edge_reveal_color`。
- 鼠标进入提示条立即恢复；标题栏悬停、任务编辑和菜单交互期间保持展开。
- 展开后开始拖动会立即解除旧停靠状态，拖离边缘后不再错误隐藏。
- macOS 使用原生 `panel` 和跨 Space 配置，在其他应用的常规全屏桌面中仍可访问。

`v0.1.3` 的任务详情与紧凑模式能力继续保持：

- Task 领域快照增加必填 `details`，Today/Week TXT 使用显式 `  |` 详情行。
- Repository 的新增、编辑、删除和跨日归档统一按“任务根行 + 连续详情行”任务块操作。
- 今日、已完成、历史待办和历史完成记录共享详情编辑与展开交互。
- 周记、模板周报和远程 LLM 继续只投影标题，详情不会扩大远程发送范围。
- 标题栏增加整窗收起/恢复；Main Process 动态调整 BrowserWindow 高度并保护完整尺寸。
- 展开箭头统一使用同一 SVG 旋转，避免系统字体导致上下箭头尺寸不一致。

详细约束见 [`待办详情编辑与展开功能设计开发文档-v1.0.md`](./待办详情编辑与展开功能设计开发文档-v1.0.md)。
...
- 远程生成授权仍使用全局布尔值，尚未绑定服务 origin 和隐私声明版本。
- 模板、提示词、凭据与配置保存尚未形成完整跨文件事务。
...
```text
origin https://github.com/StoneNbc/To-do-for-weekly-report.git
```
...
```text
便利贴/
├── build/                         # 应用、安装包与 1x/2x 托盘图标
├── documents/                     # PRD、开发设计、协作策略、交接文档
├── resources/                     # 透明主图标、默认配置等静态资源
├── src/
│   ├── main/                      # Electron 主进程
...
6. 未知行不展示，但必须原样保留。
7. 未完成任务无限顺延并保留添加日期；今日新增使用今天，历史新增使用所选历史日期；旧格式任务的添加日期保持未知。
8. 正常完成按实际勾选日期记录。
9. 历史补录按用户选择日期写入周文件。
...
```text
# 2026-08-13
- [ ] 准备周会材料 @添加:2026-08-11
- [x] 回复客户邮件 @添加:2026-08-12 @14:20
```
...
```text
resources/app-icon-master.png   1024 × 1024 RGBA 主资源
build/icon.png                  应用、Dock、窗口和 electron-builder 输入
build/tray-icon.png             16 × 16 托盘图标
build/tray-icon@2x.png          32 × 32 高分辨率托盘变体
```

开发环境由 Main 从 `process.cwd()/build` 加载；打包时 `extraResources` 将运行时图标复制到
`process.resourcesPath`。macOS 使用 `app.dock.setIcon` 设置开发模式 Dock 图标，BrowserWindow
通过 `icon` 选项接入窗口图标。更新图标后必须完全退出已运行的单实例应用再启动开发命令。
...
- 在 DMG、ZIP、Dock、Finder 和系统托盘中验收 electron-builder 生成的最终平台图标。
...
当前远程仓库为 `StoneNbc/To-do-for-weekly-report`。修改远端、默认分支或发布配置前仍需项目所有者明确授权。
...
1. 明确正式 appId、产品名和版本号。
2. 配置 macOS 签名与公证。
3. 在 Windows 10/11 实机完成 NSIS 构建和图标验收。
...
1. 将远程生成授权绑定服务 origin 与隐私声明版本，切换服务器后重新确认。
2. 为模板、提示词、凭据与配置保存增加分阶段提交或失败回滚。
3. 在 macOS 与 Windows 验证 safeStorage、HTTP/HTTPS、自定义服务和主流国产模型。
4. 保持默认本地，继续限制 Renderer 网络和远程发送范围。
...
| `pnpm dev` 仍显示旧图标    | 完全退出已有单实例应用，再启动；确认 `build/icon.png` 已更新 |
