...
> 增量说明（2026-08-17）：双模板、提示词、凭据安全存储、OpenAI 兼容远程生成和草稿预览已按《自定义周报模板与远程LLM开发设计-v1.0》落地；本文件继续描述任务、归档、TXT 保真与基础 Electron 架构。

> 增量说明（2026-09-04）：macOS 全屏可见性使用 `setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true })`，仅在“保持置顶”和“在全屏应用中显示”同时开启时生效；关闭置顶会同步关闭全屏显示配置。

## 1. 文档目的

> 项目管理增量（2026-09-09，已实现，未发布）：[项目管理开发文档](项目管理开发文档-v1.0.md)补充本文件 §6–17、§19、§21、§25，描述项目名字段、格式声明与旧数据升级、统一业务写入协调、跨文件改名恢复及契约联动。项目功能的技术差异以该增量文档为准；相关接口与恢复机制已落地，验证范围见 PROJECT_STATE。
...
export interface TodayTaskView {
  ...
  content: string;
  completed: boolean;
  addedDate?: string; // YYYY-MM-DD；旧任务可未知
...
- 空任务无效。
- `@添加:YYYY-MM-DD` 是任务添加日期的保留语法；旧任务允许没有该字段。
...
interface ArchiveService {
  reconcileToToday(
    trigger: 'startup' | 'midnight' | 'resume' | 'before-mutation',
...
未完成任务记录添加日期，但 `today.txt` 仍只有一个当前文件日期：
...
- 历史模式新增项写入全局 `today.txt`，添加日期使用所选历史日期，并立即包含在当前组合快照中。
- 不允许在历史周文件中创建 `[ ]` 项。
- 历史模式允许完成符合日期约束的全局待办，也允许撤销历史完成并将事项放回 `today.txt`。
...
```typescript
type ApiErrorCode =
  'INVALID_INPUT' | 'FILE_CHANGED' | 'NOT_FOUND' | 'INVALID_FILE' | 'IO_ERROR' | 'INTERNAL_ERROR';
...
- macOS 的便利贴使用原生 `panel` 窗口类型和 `floating` always-on-top level；用户开启全屏显示后，再启用跨 Space 与 `visibleOnFullScreen` 行为。贴边隐藏时原生窗口本身缩为 2 DIP，避免系统切换 Space 后把越界的大窗口搬回可见区域；提示条颜色由独立配置 `edge_reveal_color` 控制。
