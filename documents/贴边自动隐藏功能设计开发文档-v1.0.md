...
- 可见厚度固定为 2 DIP。
- 颜色由 `edge_reveal_color` 独立配置；设置页提供预设色和系统自定义取色器。
...
左右隐藏通过把原生窗口宽度直接缩为 2 DIP 实现：

```text
左侧隐藏 x = workArea.x，width = 2
右侧隐藏 x = workArea.x + workArea.width - 2，width = 2
```

窗口的 y、height 保持不变；y 必须限制在当前工作区内，保证提示条具有可操作高度。完整宽度只保存在 `visibleBounds`，恢复时一次性还原。
...
隐藏前临时把窗口最小宽高降为 2，并禁用用户缩放；恢复时重新应用正常模式或紧凑模式对应的最小尺寸和 `resizable` 状态。

这一实现对用户仍表现为顶部只留 2 DIP 提示条，同时避开 macOS 菜单栏坐标限制。
...
事件会先合并，再按 §8.4 恢复停靠状态；不影响布局的 metrics 事件忽略。

### 8.4 显示器切换保留隐藏状态

接入、断开或调整显示器时，优先保留用户当前停靠和隐藏状态；只有无法安全停靠时才恢复可见。

#### 行为规则

| 变化前状态／变化条件 | 结果 |
| --- | --- |
| 已隐藏，原显示器存在且原边缘仍安全 | 保留显示器和边缘，更新位置后保持 2 DIP 提示条，不先展开 |
| 已停靠展开，原边缘仍安全 | 保持展开；鼠标在窗外且无交互阻止时，重新按现有 500ms 延迟隐藏 |
| 所在显示器被移除 | 迁移到当前主屏，尝试同侧边缘；安全时保留隐藏／展开状态 |
| 同侧边缘成为内侧接缝，或无法容纳可操作窗口 | 恢复为可见、未停靠窗口；不自动改到其他边缘 |
| 原本未停靠 | 仅做现有可见区域恢复，不因显示器变化自动吸附 |
| 窗口通过关闭按钮隐藏或已最小化 | 保持原生隐藏／最小化状态，不调用 show、showInactive、restore 或 focus 将其唤出 |
| 原显示器重新接入 | 保留当前所在显示器，不自动迁回旧屏幕 |

鼠标若恰好因屏幕重排落在新的提示条内，不将坐标变化当作主动悬停唤出：须先离开提示条，再进入才展开。正常展开窗口仍按实时鼠标位置决定是否隐藏。托盘主动唤起继续即时生效。

#### 几何与状态恢复

1. 运行时缓存上一次稳定的显示器工作区；事件到来时保留原 phase、edge、displayId、完整可见边界和紧凑模式信息，不从 2 DIP 原生窗口反推完整尺寸。
2. 原 displayId 仍存在时使用原屏；不存在时选当前主屏，主屏不可用时选首个有效屏。同屏坐标变化和跨屏迁移均以沿边可移动范围的相对位置映射（左／右边映射纵向，顶边映射横向），再限制在目标工作区内；原可移动范围为零时使用起点。
3. 使用目标工作区限制尺寸，保留紧凑模式；紧凑窗口按紧凑高度定位，同时维护供展开使用的完整尺寸。尺寸规则须与现有最小尺寸策略一致，极小工作区使用可操作回退。
4. 复用 `snapNoteToEdge()`、`isExternalNoteEdge()` 和 `getHiddenNoteBounds()`，只尝试原边缘。安全时直接设置最终隐藏或展开边界，不经过“先完整展开再隐藏”的路径。
5. 无安全候选时恢复完整可见尺寸策略、清除停靠，再使用 `restoreVisibleBounds()` 找回窗口。原生关闭隐藏／最小化状态优先于显示器恢复，不得被此步骤改变。
6. 恢复指针轮询和适用的隐藏计时；只保存完整可见边界，不持久化显示器 ID、隐藏状态或迁移历史。自动隐藏开关关闭期间只执行普通可见区域恢复。

#### 事件合并与用户操作优先级

- 合并 `display-added`、`display-removed` 与影响布局的 metrics（`bounds`、`workArea`、`scaleFactor`、`rotation`）；无关 metrics 忽略。
- 采用尾部 300ms 防抖、最长 1500ms 等待。每批使用最新屏幕布局，并以本批首次事件前的稳定停靠状态作为恢复意图，避免连续事件先清掉隐藏状态。
- 恢复期间暂停旧自动隐藏、移动检测、指针轮询和边界保存任务，避免系统重排引发的 move／resize 覆盖快照；程序化 setBounds 保留现有事件保护。
- 托盘主动显示、关闭窗口、手动拖动、切换紧凑模式或关闭自动隐藏设置时，取消旧恢复计划并以最新用户操作为准；退出时清理新增计时器。
- 暂时没有有效显示器时不清空快照、不写入兜底坐标，等待后续有效布局事件；有有效屏幕但停靠恢复失败时采用可见回退。日志记录恢复前后状态、目标屏和回退原因。

#### 实施范围与验证

- Main 窗口管理负责事件合并、快照、原生可见性保护和应用结果；几何规划放在平台纯函数中。复用现有边缘计算，不改变 Renderer／IPC／配置格式。
- 当前自动化覆盖：相对位置映射、负坐标、接缝拒绝、无关屏幕接入保持隐藏、原屏移除后迁移并保持隐藏、接缝可见回退。连续真实系统事件、缩放、旋转、紧凑状态和原生隐藏／最小化留给人工验收。
- 人工验收：隐藏后接入显示器不展开；隐藏后拔掉所在屏幕在主屏显示提示条且可唤出；双屏接缝安全回退；连续插拔／缩放／旋转后仍能隐藏和拖动；关闭隐藏与紧凑模式在切屏后保持正确行为。
- 实现与状态机、窗口模块索引、README、CHANGELOG 和 PROJECT_STATE 保持同步。
...
stateDiagram-v2
    ...
    Hidden --> Hidden: 显示器变化且原边缘仍安全
    DockedVisible --> DockedVisible: 显示器变化且原边缘仍安全
    Hidden --> Undocked: 关闭设置或显示器变化后边缘不安全
    DockedVisible --> Undocked: 显示器变化后边缘不安全
```
...
隐藏时不再把完整窗口移动到屏幕外。左、右分别把原生窗口宽度直接缩为 2 DIP，顶部把原生窗口高度缩为 2 DIP。这样 macOS 在切换桌面或全屏 Space 时即使重新约束窗口位置，也不会把完整的透明/空白窗口搬回屏幕。

### 10.3 建议常量

```typescript
export const EDGE_SNAP_THRESHOLD = 12;
export const EDGE_REVEAL_SIZE = 2;
...
- 旧配置缺少 `edge_reveal_color` 时按 `#92400E` 补齐并写回。
- 非布尔值按 `false` 回退并记录字段校验日志。
- 提示条颜色只接受规范化后的六位十六进制 RGB；非法值回退默认色并记录日志。
...
edgeRevealColor ↔ edge_reveal_color
```
...
- 紧凑模式停靠顶部时，仍缩小为 2 DIP 高度。
- 隐藏状态点击托盘、鼠标进入提示条或打开菜单时，先恢复当前紧凑/正常尺寸。
- 切换紧凑状态前先恢复隐藏窗口；切换完成后重新按原方向吸附。
- `#expandedNoteBounds` 始终保存正常模式完整尺寸，不被 2 DIP 顶部提示条覆盖。

### 15.2 置顶

- 自动隐藏不修改 `always_on_top` 设置。
- `show_on_fullscreen` 默认开启，但只有 `always_on_top` 同时开启时才生效。
- macOS 的便利贴窗口使用原生 `panel` 类型，获得跨全屏 Space 的窗口能力；设置生效时仍调用 `setVisibleOnAllWorkspaces(true, { visibleOnFullScreen: true })` 明确启用跨桌面显示。
- 全屏显示生效时，贴边隐藏不调用会把原生窗口送到后层的 `blur()`，而以 `showInactive()` 保持提示条可见且不抢占当前应用焦点。
...
即使旧版本或异常路径把隐藏坐标写入配置，现有 `restoreVisibleBounds()` 仍应把不足最小可见区域的窗口恢复到主屏。新增测试需要覆盖 2 DIP 提示条坐标的恢复兜底。
...
- 增加 `edge_auto_hide`、`edge_reveal_color` 默认值、解析和设置映射。
- 完成负坐标、多显示器和旧配置测试。
...
6. 隐藏时拔掉所在显示器，窗口迁移到主屏同侧并保持提示条。
7. 正常与紧凑模式分别停靠。
8. 打开菜单、编辑任务和填写新增输入时不隐藏。
9. 托盘单击、单实例启动和应用激活可以唤回。
10. 重启后保存的是完整窗口尺寸，不是 2 DIP 或屏幕外坐标。
...
2026-09-04 验收记录：自动化质量门禁全部通过；macOS 实机确认提示条、可拖拽标题栏悬停保持展开，以及展开后拖离边缘立即解除隐藏状态。提示条随后按反馈由 4 DIP 调整为 2 DIP，并增加独立颜色配置。Windows 与复杂多显示器布局仍需在对应环境补充平台验收。
...
- [ ] 停靠后鼠标离开 500 ms 才隐藏。
- [ ] 隐藏时仅保留 2 DIP 提示条，颜色与设置页保存值一致。
- [ ] 鼠标进入提示条立即恢复完整窗口。
- [ ] 菜单、任务编辑、文本输入和保存期间不会隐藏。
- [ ] 底部边缘不会触发自动隐藏。
- [ ] 多显示器内侧接缝不会把窗口隐藏到相邻显示器。
- [ ] 显示器变化后安全边缘保持隐藏，接缝边缘恢复为完整可见窗口。
- [ ] 托盘、单实例和应用激活可以恢复隐藏窗口。
- [ ] 正常模式和紧凑模式都能正确停靠与恢复。
- [ ] 隐藏矩形和 2 DIP 宽高不会写入 `window_bounds`。
- [ ] 重启后不会出现窗口丢失、仅剩 2 DIP 宽高或无法缩放。
- [ ] macOS 顶部隐藏不尝试越过系统菜单栏。
- [ ] macOS 切换到其他普通桌面或应用全屏 Space 后，2 DIP 提示条仍可见、可唤出。
- [ ] macOS 切换 Space 后，左右侧隐藏不会出现完整尺寸白色空窗。
...
缓解：顶部统一缩小为 2 DIP 高度；恢复时严格重设最小尺寸与可缩放状态。
...
用户关闭“保持置顶”后，2 DIP 提示条可能被普通窗口遮挡。

缓解：这是置顶设置的自然结果；托盘“显示便利贴”、单实例唤起和应用激活始终可恢复。不开启自动隐藏仍是默认行为。

### 23.5 提示条过窄

2 DIP 在高分辨率设备上仍使用 DIP，但对部分用户可能不易命中。

缓解：整条边都可触发，并由主进程轮询真实鼠标坐标扩大触发可靠性；当前按用户反馈固定为 2 DIP，后续再评估是否提供可配置宽度。

### 23.6 macOS 回收屏幕外窗口

普通窗口跨 Space 时，macOS 可能把大部分位于屏幕外的窗口重新约束到可见区域，导致隐藏页面只绘制提示条、但完整原生窗口以白色空窗出现。

缓解：隐藏态使用真实的 2 DIP 原生窗口尺寸，不依赖负坐标或超出工作区的窗口位置；macOS 采用 `panel` 类型承载全屏 Space 悬浮。
